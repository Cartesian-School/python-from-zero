...
|---|---|---|---|
| P0-01 | `pyproject.toml` ошибочно связывался с созданием import package. | В `scripts/build_chapter_23.py` и сгенерированной `23-05-pyproject-toml.html` разделены import package `src/safesort`, project source tree, distribution package/archive и installed project; показаны src-layout и `pip install -e .`. | `scripts/validate_chapter23_outputs.py`; editable-install и import smoke test в CI; полный build. |
