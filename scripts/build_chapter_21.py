...
def build_01() -> None:
    ...
    18) здесь не случайны: класс-описание типа врага и явные состояния игры — тот же приём, что
...
def build_03() -> None:
    ...
    {callout(
        ...
        "себя как 1/0 (глава 4), поэтому разность даёт ровно то, что нужно: 1, если зажата только "
...
def build_14() -> None:
    ...
    {callout(
        ...
        "<code class=\"inline\">@dataclass</code> из главы 14 автоматически создаёт "
        "конструктор для четырёх полей. Параметр <code class=\"inline\">frozen=True</code> "
        "запрещает менять эти поля после создания объекта: общая спецификация scout или fighter "
        "остаётся неизменной, а координаты и текущая скорость хранятся уже в каждом экземпляре "
        "<code class=\"inline\">Enemy</code>. Чтобы добавить тип врага в упражнении 21.25, "
        "создайте новый <code class=\"inline\">EnemySpec</code>, а не изменяйте существующий.",
    )}
...
def build_16() -> None:
    ...
    <h2>Побег врага за нижнюю границу</h2>
    <p>У игры есть второй, отдельный источник потери жизней. Враг считается сбежавшим, только
    когда его спрайт целиком пересёк нижнюю границу игрового поля
    (<code class="inline">vrag.rect.top &gt; self.playfield.bottom</code>). Каждый такой враг
    удаляется и стоит игроку одну жизнь:</p>
    {code_block(
    ...
    {callout(
        ...
        "Побег — не удар по кораблю, поэтому временная неуязвимость его не блокирует. Если в "
        "одном обновлении поле покинули два врага, игра списывает две жизни; если жизней не "
        "осталось, состояние переходит в <code class=\"inline\">GAME_OVER</code>. Это "
        "намеренное правило: столкнувшиеся одновременно враги образуют одно событие удара, а "
        "каждый пропущенный враг — отдельный результат, который учитывается ровно один раз.",
    )}
    ...
    out = render_page(
        ...
        description="Система жизней, защита от повторного урона при столкновениях, временная неуязвимость и отдельное правило потери жизней за сбежавших врагов.",
        depth=2,
        ...
        lede="Столкновения и побег врага — два разных источника потери жизней с разными правилами.",
        body_html=body,
...
def build_23() -> None:
    ...
    <h2>Встроенный отладочный оверлей</h2>
    <p>Нажмите <kbd>F3</kbd>, чтобы включить или выключить отладочный оверлей финальной игры.
    Он рисует красные границы <code class="inline">Rect</code> корабля, врагов и пуль, а внизу
    показывает фактический FPS. Так можно увидеть реальные хитбоксы при разборе столкновений и
    отличить просадку частоты кадров от ошибки в расчёте движения через
    <code class="inline">dt</code>. Оверлей меняет только отрисовку: состояние игрового мира,
    таймеры и столкновения остаются прежними.</p>
