...
def build_12() -> None:
    ...
    явно проверяет это: если точка начала и точка отпускания почти совпадают, линия, прямоугольник
    или овал попросту не создаётся — вместо невидимой фигуры нулевого размера.</p>
...
def build_13() -> None:
    ...
    {class_diagram(
        ...
        [],
        caption="Один объект вместо восьми отдельных переменных — те же данные, но с понятной границей.",
    )}

    <h2 id="enum">Новое: Enum — заранее известный список вариантов</h2>
    <p>Пока инструмент хранится строкой, <strong>любое</strong> слово выглядит допустимым:
    <code class="inline">"pryamougolnik"</code>, <code class="inline">"pryamougolnick"</code>,
    <code class="inline">"квадрат"</code>. Python не возражает — он и не знает, какие варианты вы
    считаете правильными. Опечатка всплывёт гораздо позже и совсем не там: значение просто не
    совпадёт ни с одной веткой <code class="inline">if</code>, и нажатие мыши тихо перестанет
    что-либо рисовать.</p>
    <p>Но у инструмента конечное, заранее известное число вариантов — карандаш, линия,
    прямоугольник, овал, ластик. Для таких случаев в Python есть <strong>перечисление</strong>
    (<code class="inline">Enum</code> из модуля <code class="inline">enum</code>): вы один раз
    выписываете все допустимые значения, и дальше пользуетесь только ими.</p>
    {code_block(
        "enum_vpervye.py",
        ...
        "print(Tool.RECTANGL)           # AttributeError — опечатка видна СРАЗУ\n",
    )}
    <p>Три вещи, которые стоит запомнить про <code class="inline">Enum</code>:</p>
    <ul>
      <li><code class="inline">Tool.RECTANGLE</code> — это <strong>член</strong> перечисления,
      отдельное значение, а не строка. Его удобно передавать и хранить;</li>
      <li><code class="inline">.value</code> достаёт строку, которую вы за ним закрепили — она
      пригодится, когда фигуру нужно будет записать в JSON (раздел 18.30);</li>
      <li>члены сравнивают через <code class="inline">is</code>, а не
      <code class="inline">==</code>: член перечисления существует ровно в одном экземпляре,
      поэтому <code class="inline">tool is Tool.ERASER</code> — самая точная и быстрая проверка.</li>
    </ul>
    <p>Главное отличие от строки: <code class="inline">Tool.RECTANGL</code> — это
    <code class="inline">AttributeError</code> при первом же обращении, а
    <code class="inline">"pryamougolnick"</code> — совершенно «нормальная» строка, о которой
    Python промолчит.</p>

    <h2>Собираем DrawingState</h2>
    <p>Теперь соберём оба инструмента вместе: <code class="inline">Enum</code> для инструмента и
    <code class="inline">@dataclass</code> (раздел 14.23) для самого объекта состояния —
    он избавляет от рукописного <code class="inline">__init__</code> и сразу даёт понятный
    <code class="inline">repr</code>:</p>
    {code_block(
        "drawing_state.py",
    ...
    {callout(
        ...
        "Если вариант всего один или их список постоянно меняется — обычной строки достаточно, "
        "перечисление только добавит церемоний.",
    )}
...
def build_14() -> None:
    ...
    <ul>
      ...
      <li>кружок ничего не знает о том, где курсор был мгновение назад, — путь между двумя
      соседними событиями просто теряется.</li>
    </ul>
    ...
    {callout(
        ...
        "Начиная с этого раздела примеры пишутся как методы: "
        "<code class=\"inline\">def on_drag(self, event)</code>, "
        "<code class=\"inline\">self.state</code>, <code class=\"inline\">self.canvas</code>. "
        "Все они принадлежат классу <code class=\"inline\">PaintApp</code>, который мы соберём "
        "целиком в разделе 18.29 — до тех пор достаточно читать "
        "<code class=\"inline\">self.state.color</code> как «текущий цвет приложения».",
    )}
    ...
    {comparison_table(
        ...
        [
            ...
            ["<code class=\"inline\">width</code>", "толщина штриха; чем она больше, тем заметнее была бы «зубчатость» стыков без capstyle"],
        ],
    )}
    {callout(
        "warning",
        "А как же smooth=True? Здесь он не сработает",
        "У <code class=\"inline\">create_line()</code> есть параметр "
        "<code class=\"inline\">smooth=True</code> — Tk сглаживает ломаную, проводя через её "
        "точки кривую. Но сглаживать он умеет только <strong>внутренние</strong> точки одной "
        "линии, а каждый наш отрезок состоит ровно из ДВУХ точек — между ними нет ни одного "
        "угла. Добавить <code class=\"inline\">smooth=True</code> в этот код можно, и Tk его "
        "молча примет, но на экране не изменится ровно ничего. Параметр стал бы полезен, если "
        "бы весь штрих был ОДНОЙ длинной ломаной — такой вариант описан ниже.",
    )}
    {callout(
        "tip",
        "Куда это можно развить: один штрих — одна ломаная",
        "Сейчас каждое движение мыши создаёт отдельный элемент Canvas. Можно вместо этого "
        "создать линию один раз на <code class=\"inline\">&lt;Button-1&gt;</code> и на каждом "
        "движении дописывать к ней новую пару координат через "
        "<code class=\"inline\">coords()</code> (раздел 18.18): тогда длинный штрих — это один "
        "элемент вместо сотни, а <code class=\"inline\">smooth=True</code> наконец получает "
        "внутренние точки, которые действительно можно сгладить. В этой главе оставлен более "
        "простой вариант с отдельными отрезками — он короче и его легче отлаживать.",
    )}
...
def build_18() -> None:
    ...
    ], caption="Создать один раз → много раз обновить → зафиксировать (CREATE ONCE → UPDATE MANY TIMES → COMMIT) — один и тот же элемент, а не поток одноразовых.")}
    {code_block(
        "zhivoe_prevyu.py",
        "def on_press(self, event):\n"
    ...
    out = render_page(
        ...
        description="Создать один раз, много раз обновить, зафиксировать — обновление одного элемента Canvas вместо пересоздания на каждое движение мыши.",
        depth=2,
...
def build_19() -> None:
    body = f"""
    <h2>Четыре способа обратиться к уже нарисованному</h2>
    {comparison_table(
        ["Метод", "Что меняет"],
        [
...
def build_21() -> None:
    ...
    {image_figure(f"{IMG}/custom-color-result.png", "Реальное окно: текущий цвет в панели инструментов изменился на нестандартный оттенок, выбранный через диалог", "Реальное окно: результат выбора пользовательского цвета — сам диалог в кадр не попал, но эффект от него виден по-настоящему.", width=420)}
...
def build_26() -> None:
    ...
    {callout(
        ...
        "после очистки внешне не сделает НИЧЕГО (убирать из пустого документа нечего), зато "
        "положит устаревшее действие в <code class=\"inline\">redo_stack</code>. И тогда Ctrl+Y "
        "вернёт на холст фигуры, которые пользователь только что удалил «Очистить». "
...
def build_30() -> None:
    ...
    <h2 id="povrezhdennyj-fajl">А если файл повреждён?</h2>
    <p>Открывает файл пользователь — значит, выбрать он может что угодно: чужой JSON, картинку,
    недописанный при сбое файл, отредактированный вручную рисунок с опечаткой. Глава 15
    (раздел 15.25) уже показывала, что <code class="inline">json.load()</code> сообщает об этом
    через <code class="inline">json.JSONDecodeError</code>. Здесь к нему добавляются ещё
    несколько возможностей: ключа <code class="inline">"items"</code> может не быть вовсе, у
    фигуры может не хватать поля, а цвет может оказаться строкой, которой Tk не знает.</p>
    <p>Важно не просто «не упасть», а не потерять рисунок, над которым пользователь уже работал.
    Поэтому загрузка разбирает файл в <strong>локальный</strong> список и заменяет документ
    только тогда, когда разбор целиком удался:</p>
    {code_block(
        "zagruzka_bezopasno.py",
    ...
    {callout(
        ...
        "Соблазн написать <code class=\"inline\">self.document = [...]</code> сразу, а "
        "<code class=\"inline\">try</code> навесить только на "
        "<code class=\"inline\">json.loads()</code>. Тогда файл с неизвестным цветом пройдёт "
        "разбор, документ уже будет заменён — и упадёт "
        "<code class=\"inline\">render_document()</code>. Итог: прежний рисунок потерян, "
        "история отмены очищена, а каждая следующая попытка нарисовать хоть что-нибудь снова "
        "падает, потому что перерисовка каждый раз натыкается на ту же плохую фигуру. Проверка "
        "ДО присваивания стоит одной лишней переменной и спасает работу пользователя.",
    )}
    {callout(
        "info",
        "Проверять цвет умеет только холст",
        "<code class=\"inline\">Shape</code> сознательно ничего не знает о Tkinter "
        "(раздел 18.29), поэтому строку цвета он проверить не может — для него "
        "<code class=\"inline\">\"#2563eb\"</code> и <code class=\"inline\">\"не-цвет\"</code> "
        "одинаково похожи на цвет. А вот <code class=\"inline\">canvas.winfo_rgb(color)</code> "
        "спрашивает у самого Tk и бросает <code class=\"inline\">TclError</code>, если такого "
        "цвета нет. Разделение обязанностей сохранено: модель проверяет структуру, холст — то, "
        "что относится к рисованию.",
    )}
...
def build_31() -> None:
    ...
    {debug_lab(
        9,
        "Undo убирает только последний отрезок штриха, а не весь штрих",
    ...
    {debug_lab(
        ...
        ["# После 'Очистить' нажатие Ctrl+Z не делает ничего видимого —", "# а следующий Ctrl+Y возвращает на холст фигуры, которые", "# пользователь только что удалил кнопкой 'Очистить'."],
        "Ошибка тихая: <code class=\"inline\">del document[len(document) - len(shapes):]</code> "
        "на пустом документе — это <code class=\"inline\">del document[-3:]</code>, что ничего "
        "не удаляет и ничего не ломает. Зато отменённое «действие» уезжает в "
        "<code class=\"inline\">redo_stack</code>, и следующий Ctrl+Y возвращает на холст "
        "фигуры, стёртые кнопкой «Очистить». История описывает документ, которого больше нет: "
        "как только <code class=\"inline\">document</code> очищается, "
        "<code class=\"inline\">undo_stack</code> и <code class=\"inline\">redo_stack</code> "
        "обязаны очиститься вместе с ним (раздел 18.26).",
    ...
    {debug_lab(
        ...
        "    self.undo_stack.clear()\n"
        "    self.redo_stack.clear()\n"
        "    self.render_document()   # без этой строки экран не узнает о загрузке\n"
        "    # (полная версия с обработкой повреждённого файла — в разделе 18.30)\n",
    )}

    {debug_lab(
        16,
        "Повреждённый файл стирает текущий рисунок",
        "zagruzka_bez_proverki.py",
        "def load_from_path(self, path):\n"
        "    data = json.loads(path.read_text(encoding=\"utf-8\"))\n"
        "    self.document = [Shape.from_dict(i) for i in data[\"items\"]]  # уже заменили!\n"
        "    self.undo_stack.clear()\n"
        "    self.render_document()   # ...и только здесь падаем на плохом цвете\n",
        ["# Открыли чужой JSON — рисунок, над которым работали час, исчез,", "# Ctrl+Z его не возвращает, а каждая новая линия снова падает:", "# _tkinter.TclError: unknown color name \"not-a-color\""],
        "Документ заменён ДО того, как стало известно, что файл рисуется. Разбирайте файл в "
        "локальную переменную и присваивайте <code class=\"inline\">self.document</code> только "
        "после успеха (раздел 18.30) — иначе одна попытка открыть не тот файл уносит и рисунок, "
        "и историю, и возможность рисовать дальше.",
        "zagruzka_s_proverkoj.py",
        "def load_from_path(self, path):\n"
        "    try:\n"
        "        data = json.loads(path.read_text(encoding=\"utf-8\"))\n"
        "        shapes = [Shape.from_dict(i) for i in data[\"items\"]]\n"
        "        for shape in shapes:\n"
        "            self.canvas.winfo_rgb(shape.color)\n"
        "    except (json.JSONDecodeError, KeyError, TypeError, ValueError, tk.TclError) as exc:\n"
    ...
    out = render_page(
        ...
        description="Шестнадцать разобранных багов приложения для рисования на Canvas: координаты, превью, undo/redo, диалоги, JSON.",
        depth=2,
