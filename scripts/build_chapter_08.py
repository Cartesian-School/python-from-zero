...
def build_18() -> None:
    ...
    {callout(
        ...
        "Мы ещё не систематизировали циклы — это тема главы 10. Здесь вводится один полный "
        "шаблон, достаточный для работы со строкой: "
    ...
    <h2>Практический пример без новых условий</h2>
    {code_block("simvoly_verhnim_registrom.py", 'text = "Python"\nfor ch in text:\n    print(ch.upper())\n# P\n# Y\n# T\n# H\n# O\n# N\n')}
    <p>Тело цикла — строка с отступом — выполняется один раз для каждого символа. Условия
    <code class="inline">if</code> появятся в главе 9, а глава 10 систематизирует циклы и
    покажет накопители, фильтрацию и управление повторением.</p>

    {exercise(1, "Печатаем символы", "Переберите строку «миссисипи» в цикле for и выведите каждый символ в верхнем регистре.")}
...
def build_23() -> None:
    ...
    <p>Считаем общее количество слов с помощью <code class="inline">split()</code>
    (раздел 8.10). Частоты отдельных слов потребуют словаря из главы 11 и появятся в
    расширенной версии проекта позже:</p>
    {code_block(
        "schetchik_slov.py",
    ...
    {callout(
        "info",
        "[[icon:launch]] Забегаем вперёд",
        "Версия «слово → число повторений» намеренно отложена до главы 11: сначала нужно "
        "освоить условия, циклы и словари. Здесь проект использует только уже введённые "
    ...
    {exercise(2, "Непустые слова", "Введите предложение с несколькими пробелами и убедитесь, что split() возвращает только слова. Выведите список слов и их общее количество.")}
