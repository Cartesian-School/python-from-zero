...
PAGES = [
    ...
    ("22-10-https.html", "HTTPS: как защищается соединение"),
    ("22-11-flask-marshrutizaciya.html", "Маршрутизация во Flask: как запрос попадает в обработчик"),
    ("22-12-jinja-shablony.html", "Шаблоны Jinja: HTML и данные из Python"),
    ("22-13-formy.html", "HTML-формы: отправляем данные на сервер"),
    ("22-14-staticheskie-fajly.html", "Статические файлы: CSS, изображения и JavaScript"),
    ("22-15-json.html", "JSON: формат обмена данными"),
    ("22-16-api.html", "API: ответы с данными вместо HTML"),
    ("22-17-veb-frejmvorki.html", "Основные веб-фреймворки Python"),
    ("22-18-flask-django-fastapi.html", "Flask, Django и FastAPI: как выбрать подход"),
    ("22-19-wsgi-asgi.html", "WSGI и ASGI: связь приложения с сервером"),
    ...
    ("22-23-sqlite.html", "SQLite: база данных без отдельного сервера"),
    ("22-24-postgresql-mysql-sqlite.html", "PostgreSQL, MySQL/MariaDB и SQLite: основные различия"),
    ("22-25-nosql.html", "Нереляционные базы данных: основные модели и применение"),
    ("22-26-orm.html", "ORM: работа с базой через объекты Python"),
    ("22-27-migracii-shemy.html", "Миграции схемы: как изменяется структура базы"),
    ("22-28-perenos-dannyh.html", "Перенос данных между базами с помощью Python и JSON"),
    ("22-29-flask-sqlite.html", "Flask и SQLite: сохраняем задачи в базе данных"),
    ("22-30-cookies-session.html", "Cookie и сессии: как сохраняется состояние между запросами"),
    ...
    ("22-33-testiruem-flask.html", "Автоматические тесты Flask-приложения"),
    ("22-34-razvyortyvanie.html", "Развёртывание Flask-приложения: от разработки к рабочей среде"),
...
def build_opener() -> None:
    out = render_chapter_opener(
        ...
        sections=[
            ...
            ChapterSectionLink("22.10", "HTTPS: как защищается соединение", "22-10-https.html"),
            ChapterSectionLink("22.11", "Маршрутизация во Flask: как запрос попадает в обработчик", "22-11-flask-marshrutizaciya.html"),
            ChapterSectionLink("22.12", "Шаблоны Jinja: HTML и данные из Python", "22-12-jinja-shablony.html"),
            ChapterSectionLink("22.13", "HTML-формы: отправляем данные на сервер", "22-13-formy.html"),
            ...
            ChapterSectionLink("22.16", "API: ответы с данными вместо HTML", "22-16-api.html"),
            ChapterSectionLink("22.17", "Основные веб-фреймворки Python", "22-17-veb-frejmvorki.html"),
            ChapterSectionLink("22.18", "Flask, Django и FastAPI: как выбрать подход", "22-18-flask-django-fastapi.html"),
            ChapterSectionLink("22.19", "WSGI и ASGI: связь приложения с сервером", "22-19-wsgi-asgi.html"),
            ...
            ChapterSectionLink("22.23", "SQLite: база данных без отдельного сервера", "22-23-sqlite.html"),
            ChapterSectionLink("22.24", "PostgreSQL, MySQL/MariaDB и SQLite: основные различия", "22-24-postgresql-mysql-sqlite.html"),
            ChapterSectionLink("22.25", "Нереляционные базы данных: основные модели и применение", "22-25-nosql.html"),
            ChapterSectionLink("22.26", "ORM: работа с базой через объекты Python", "22-26-orm.html"),
            ChapterSectionLink("22.27", "Миграции схемы: как изменяется структура базы", "22-27-migracii-shemy.html"),
            ChapterSectionLink("22.28", "Перенос данных между базами с помощью Python и JSON", "22-28-perenos-dannyh.html"),
            ChapterSectionLink("22.29", "Flask и SQLite: сохраняем задачи в базе данных", "22-29-flask-sqlite.html"),
            ChapterSectionLink("22.30", "Cookie и сессии: как сохраняется состояние между запросами", "22-30-cookies-session.html"),
            ...
            ChapterSectionLink("22.33", "Автоматические тесты Flask-приложения", "22-33-testiruem-flask.html"),
            ChapterSectionLink("22.34", "Развёртывание Flask-приложения: от разработки к рабочей среде", "22-34-razvyortyvanie.html"),
...
def build_01() -> None:
    ...
    {callout(
        ...
        "«тыл»). В типичном веб-приложении Python на сайтах работает именно как бэкенд: код "
        "выполняется на сервере, а браузеру отправляется уже готовый результат. Python можно "
        "запустить и в самом браузере — например, через окружение на основе WebAssembly вроде "
        "Pyodide, как в интерактивной практике этой книги, — но это отдельная модель "
        "выполнения, а не стандартная архитектура серверного веб-приложения, которую разбирает "
        "эта глава.",
    )}
...
def build_03() -> None:
    body = f"""
    <p>Без собственных стилей браузер применяет стандартное оформление: обычный шрифт, чёрный
    текст, минимальные отступы. За внешний вид страницы отвечает <strong>CSS</strong>
...
def build_04() -> None:
    ...
    статьями хорошо работают на одном HTML (плюс CSS для оформления) — браузер отображает
    ...
    out = render_page(
        ...
        lede="HTML задаёт структуру, CSS — оформление, а JavaScript позволяет странице реагировать на действия пользователя и изменять содержимое прямо в браузере.",
        body_html=body,
...
def build_05() -> None:
    body = f"""
    <p><strong>Flask</strong> — лёгкий веб-фреймворк для Python: он связывает адреса запросов
    с функциями Python, которые обрабатывают запрос и формируют ответ (обычно HTML-страницу).
    Сам Flask не принимает сетевые соединения напрямую — этим занимается отдельный сервер
    (раздел 22.19 объясняет эту границу подробнее); при локальном запуске Flask временно берёт
    эту роль на себя через встроенный сервер разработки. Установка — как у любой библиотеки:</p>
    ...
    {callout(
        ...
        "<code class=\"inline\">glavnaya()</code> и отправь то, что она вернёт». Этот механизм "
        "подробно рассматривается в разделе 22.11.",
    )}
    ...
    out = render_page(
        ...
        lede="Flask — лёгкий веб-фреймворк для Python: связывает адреса запросов с функциями, которые готовят ответ.",
        body_html=body,
...
def build_07() -> None:
    ...
    out = render_page(
        ...
        nav=PageNav(prev_href="22-06-itogi.html", prev_label="Первый веб-проект: что мы уже умеем", next_href="22-08-url-anatomiya.html", next_label="URL: домен, путь, параметры и фрагмент"),
    )
...
def build_08() -> None:
    ...
    out = render_page(
        ...
        nav=PageNav(prev_href="22-07-put-zaprosa.html", prev_label="Что происходит после ввода адреса в браузере", next_href="22-09-http.html", next_label="HTTP: запросы, ответы, методы и коды состояния"),
    )
...
def build_09() -> None:
    ...
    out = render_page(
        ...
        nav=PageNav(prev_href="22-08-url-anatomiya.html", prev_label="URL: домен, путь, параметры и фрагмент", next_href="22-10-https.html", next_label="HTTPS: как защищается соединение"),
    )
...
def build_10() -> None:
    body = f"""
    <p>Обычный HTTP-запрос идёт по сети без транспортного шифрования: любой, кто технически
    способен перехватить трафик между браузером и сервером (например, в общей Wi-Fi сети),
    может его прочитать или незаметно изменить. <strong>HTTPS</strong> — это HTTP поверх TLS
    (Transport Layer Security): протокол тот же, но соединение защищено.</p>
    ...
    """
    out = render_page(
        page_title="HTTPS: как защищается соединение",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="HTTPS: как защищается соединение",
        ...
        nav=PageNav(prev_href="22-09-http.html", prev_label="HTTP: запросы, ответы, методы и коды состояния", next_href="22-11-flask-marshrutizaciya.html", next_label="Маршрутизация во Flask: как запрос попадает в обработчик"),
    )
...
def build_11() -> None:
    ...
    """
    out = render_page(
        page_title="Маршрутизация во Flask: как запрос попадает в обработчик",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="Маршрутизация во Flask: как запрос попадает в обработчик",
        ...
        nav=PageNav(prev_href="22-10-https.html", prev_label="HTTPS: как защищается соединение", next_href="22-12-jinja-shablony.html", next_label="Шаблоны Jinja: HTML и данные из Python"),
    )
...
def build_12() -> None:
    ...
    """
    out = render_page(
        page_title="Шаблоны Jinja: HTML и данные из Python",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="Шаблоны Jinja: HTML и данные из Python",
        ...
        nav=PageNav(prev_href="22-11-flask-marshrutizaciya.html", prev_label="Маршрутизация во Flask: как запрос попадает в обработчик", next_href="22-13-formy.html", next_label="HTML-формы: отправляем данные на сервер"),
    )
...
def build_13() -> None:
    ...
    """
    out = render_page(
        page_title="HTML-формы: отправляем данные на сервер",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="HTML-формы: отправляем данные на сервер",
        ...
        nav=PageNav(prev_href="22-12-jinja-shablony.html", prev_label="Шаблоны Jinja: HTML и данные из Python", next_href="22-14-staticheskie-fajly.html", next_label="Статические файлы: CSS, изображения и JavaScript"),
    )
...
def build_14() -> None:
    ...
    out = render_page(
        ...
        nav=PageNav(prev_href="22-13-formy.html", prev_label="HTML-формы: отправляем данные на сервер", next_href="22-15-json.html", next_label="JSON: формат обмена данными"),
    )
...
def build_15() -> None:
    ...
    out = render_page(
        ...
        nav=PageNav(prev_href="22-14-staticheskie-fajly.html", prev_label="Статические файлы: CSS, изображения и JavaScript", next_href="22-16-api.html", next_label="API: ответы с данными вместо HTML"),
    )
...
def build_16() -> None:
    ...
    """
    out = render_page(
        page_title="API: ответы с данными вместо HTML",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="API: ответы с данными вместо HTML",
        ...
        nav=PageNav(prev_href="22-15-json.html", prev_label="JSON: формат обмена данными", next_href="22-17-veb-frejmvorki.html", next_label="Основные веб-фреймворки Python"),
    )
...
def build_17() -> None:
    ...
    дальше, полезно увидеть, какие ещё фреймворки существуют и чем они принципиально
    ...
    ], title="Flask, Django, FastAPI и Starlette")}
    ...
    <h2>Другие фреймворки Python-веба</h2>
    <p>Кроме Flask, Django, FastAPI и Starlette, в Python-вебе используются и другие проекты:
    <strong>Litestar</strong> — современный ASGI-фреймворк с акцентом на типизацию и
    производительность, <strong>Quart</strong> — фреймворк с API, похожим на Flask, но
    построенный на ASGI, <strong>Pyramid</strong> — фреймворк, применимый и для маленьких
    скриптов, и для крупных приложений, <strong>Tornado</strong> — один из первых асинхронных
    фреймворков Python, и <strong>Bottle</strong> — компактный однофайловый микрофреймворк. Ни
    один из них не рассматривается в этой главе подробно — они упомянуты, чтобы вы узнавали
    ...
    """
    out = render_page(
        page_title="Основные веб-фреймворки Python",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="Основные веб-фреймворки Python",
        ...
        nav=PageNav(prev_href="22-16-api.html", prev_label="API: ответы с данными вместо HTML", next_href="22-18-flask-django-fastapi.html", next_label="Flask, Django и FastAPI: как выбрать подход"),
    )
...
def build_18() -> None:
    ...
    {comparison_table(
        ...
        [
            ...
            ["Валидация входных данных", "Вручную или через расширения", "Через формы и сериализаторы Django/DRF", "Встроенная, на основе подсказок типов Python"],
            ["Структура проекта", "Не навязывается", "Заданная структура приложений/проектов", "Не навязывается"],
    ...
    <h2>Почему в этой главе используется Flask</h2>
    <p>Для первого знакомства с серверной веб-разработкой в этой главе используется Flask.
    У него небольшое ядро: маршрутизация, обработка запросов и ответов, а также работа с
    шаблонами остаются хорошо заметны в коде. Это удобно для учебного проекта, где важно
    увидеть основные механизмы отдельно, не разбираясь одновременно с крупным набором готовых
    подсистем. Выбор Flask здесь сделан в учебных целях и не означает, что он лучше Django или
    FastAPI для всех задач — у каждого из них есть сценарии, где он подходит лучше.</p>
    """
    out = render_page(
        ...
        nav=PageNav(prev_href="22-17-veb-frejmvorki.html", prev_label="Основные веб-фреймворки Python", next_href="22-19-wsgi-asgi.html", next_label="WSGI и ASGI: связь приложения с сервером"),
    )
...
def build_19() -> None:
    ...
    <p>В обычном развёртывании сетевые соединения принимает сервер приложений или другая
    серверная инфраструктура, а не сам фреймворк (Flask, Django, FastAPI) напрямую. Между
    фреймворком и этим сервером нужен общий протокол — соглашение о том, как именно сервер
    передаёт запрос фреймворку и получает от него ответ.</p>
    ...
    """
    out = render_page(
        page_title="WSGI и ASGI: связь приложения с сервером",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="WSGI и ASGI: связь приложения с сервером",
...
def build_20() -> None:
    ...
    <p>СУБД (система управления базами данных) предоставляет механизмы постоянного хранения,
    запросов, транзакций, ограничений и согласованной работы с данными — она решает именно эти
    проблемы. Конкретные гарантии при этом зависят от выбранной СУБД, спроектированной схемы,
    использования транзакций и кода самого приложения: база данных не устраняет проблему
    автоматически сама по себе, если ей не пользоваться правильно.</p>
    ...
    out = render_page(
        ...
        nav=PageNav(prev_href="22-19-wsgi-asgi.html", prev_label="WSGI и ASGI: связь приложения с сервером", next_href="22-21-relyacionnaya-baza.html", next_label="Реляционные базы данных: таблицы, ключи и связи"),
    )
...
def build_22() -> None:
    ...
    out = render_page(
        ...
        nav=PageNav(prev_href="22-21-relyacionnaya-baza.html", prev_label="Реляционные базы данных: таблицы, ключи и связи", next_href="22-23-sqlite.html", next_label="SQLite: база данных без отдельного сервера"),
    )
...
def build_23() -> None:
    ...
    подключается по сети. Вместо этого движок SQLite работает прямо внутри процесса
    приложения, а основная база обычно хранится в одном файле на диске (в зависимости от
    режима работы SQLite может создавать рядом и служебные файлы, например journal или WAL).
    Модуль <code class="inline">sqlite3</code> входит в стандартную библиотеку Python —
    дополнительно ничего устанавливать не нужно.</p>
    ...
    {callout(
        ...
        "основное хранилище локальных данных или как часть более крупной системы. Она хорошо "
        "подходит для локальных приложений, обучения, прототипов, тестов (раздел 22.33) и "
        "многих небольших и средних развёртываний, где её модель одновременного доступа "
        "достаточна. Клиент-серверную базу выбирают не потому, что SQLite чем-то хуже как "
    ...
    """
    out = render_page(
        page_title="SQLite: база данных без отдельного сервера",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="SQLite: база данных без отдельного сервера",
        lede="Основная база данных обычно хранится в одном файле на диске, а движок работает прямо внутри вашего приложения.",
        body_html=body,
...
def build_24() -> None:
    ...
    ], title="PostgreSQL, MySQL/MariaDB и SQLite")}
    ...
    out = render_page(
        ...
        nav=PageNav(prev_href="22-23-sqlite.html", prev_label="SQLite: база данных без отдельного сервера", next_href="22-25-nosql.html", next_label="Нереляционные базы данных: основные модели и применение"),
    )
...
def build_25() -> None:
    ...
    {capability_map([
        ("Документные", ["Пример модели: MongoDB", "Хранят документы в собственном бинарном формате (у MongoDB — BSON, структурно близкий к JSON)", "Удобны, когда форма записей может меняться"]),
        ("Ключ-значение", ["Пример модели: Redis", "In-memory хранилище структур данных с очень быстрым доступом по ключу", "Часто используется для кеша, счётчиков, очередей — как дополнение к основной базе, а не её замена"]),
    ...
    """
    out = render_page(
        page_title="Нереляционные базы данных: основные модели и применение",
        description="Документные, ключ-значение, графовые и поисковые базы данных — обзор NoSQL-семейства и когда оно действительно нужно.",
        depth=2,
        breadcrumb=[("Python с нуля", "../../index.html"), ("Глава 22", "index.html"), ("Нереляционные базы данных", "")],
        kicker="Глава 22 · Веб-разработка с Python",
        h1="Нереляционные базы данных: основные модели и применение",
        lede="NoSQL — не замена реляционным базам, а отдельный набор моделей для других задач.",
        body_html=body,
        sidebar_groups=sidebar("22-25-nosql.html"),
        nav=PageNav(prev_href="22-24-postgresql-mysql-sqlite.html", prev_label="PostgreSQL, MySQL/MariaDB и SQLite: основные различия", next_href="22-26-orm.html", next_label="ORM: работа с базой через объекты Python"),
    )
...
def build_26() -> None:
    ...
    out = render_page(
        ...
        nav=PageNav(prev_href="22-25-nosql.html", prev_label="Нереляционные базы данных: основные модели и применение", next_href="22-27-migracii-shemy.html", next_label="Миграции схемы: как изменяется структура базы"),
    )
...
def build_27() -> None:
    ...
    """
    out = render_page(
        page_title="Миграции схемы: как изменяется структура базы",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="Миграции схемы: как изменяется структура базы",
        ...
        nav=PageNav(prev_href="22-26-orm.html", prev_label="ORM: работа с базой через объекты Python", next_href="22-28-perenos-dannyh.html", next_label="Перенос данных между базами с помощью Python и JSON"),
    )
...
def build_28() -> None:
    ...
    """
    out = render_page(
        page_title="Перенос данных между базами с помощью Python и JSON",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="Перенос данных между базами с помощью Python и JSON",
        ...
        nav=PageNav(prev_href="22-27-migracii-shemy.html", prev_label="Миграции схемы: как изменяется структура базы", next_href="22-29-flask-sqlite.html", next_label="Flask и SQLite: сохраняем задачи в базе данных"),
    )
...
def build_29() -> None:
    ...
    """
    out = render_page(
        page_title="Flask и SQLite: сохраняем задачи в базе данных",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="Flask и SQLite: сохраняем задачи в базе данных",
        ...
        nav=PageNav(prev_href="22-28-perenos-dannyh.html", prev_label="Перенос данных между базами с помощью Python и JSON", next_href="22-30-cookies-session.html", next_label="Cookie и сессии: как сохраняется состояние между запросами"),
    )
...
def build_30() -> None:
    ...
    """
    out = render_page(
        page_title="Cookie и сессии: как сохраняется состояние между запросами",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="Cookie и сессии: как сохраняется состояние между запросами",
        ...
        nav=PageNav(prev_href="22-29-flask-sqlite.html", prev_label="Flask и SQLite: сохраняем задачи в базе данных", next_href="22-31-validaciya-oshibki.html", next_label="Проверка входных данных и обработка ошибок"),
    )
...
def build_31() -> None:
    ...
    out = render_page(
        ...
        nav=PageNav(prev_href="22-30-cookies-session.html", prev_label="Cookie и сессии: как сохраняется состояние между запросами", next_href="22-32-bezopasnost.html", next_label="Основы безопасности веб-приложений"),
    )
...
def build_32() -> None:
    ...
    out = render_page(
        ...
        nav=PageNav(prev_href="22-31-validaciya-oshibki.html", prev_label="Проверка входных данных и обработка ошибок", next_href="22-33-testiruem-flask.html", next_label="Автоматические тесты Flask-приложения"),
    )
...
def build_33() -> None:
    ...
    """
    out = render_page(
        page_title="Автоматические тесты Flask-приложения",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="Автоматические тесты Flask-приложения",
        ...
        nav=PageNav(prev_href="22-32-bezopasnost.html", prev_label="Основы безопасности веб-приложений", next_href="22-34-razvyortyvanie.html", next_label="Развёртывание Flask-приложения: от разработки к рабочей среде"),
    )
...
def build_34() -> None:
    ...
    """
    out = render_page(
        page_title="Развёртывание Flask-приложения: от разработки к рабочей среде",
        ...
        kicker="Глава 22 · Веб-разработка с Python",
        h1="Развёртывание Flask-приложения: от разработки к рабочей среде",
        ...
        nav=PageNav(prev_href="22-33-testiruem-flask.html", prev_label="Автоматические тесты Flask-приложения", next_href="22-35-itogovyj-proekt.html", next_label="Итоговый проект: список задач на Flask и SQLite"),
    )
...
def build_35() -> None:
    ...
    out = render_page(
        ...
        nav=PageNav(prev_href="22-34-razvyortyvanie.html", prev_label="Развёртывание Flask-приложения: от разработки к рабочей среде", next_href="22-36-chto-dalshe.html", next_label="Что изучать дальше в веб-разработке на Python"),
    )
