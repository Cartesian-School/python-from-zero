...
def build_05() -> None:
    ...
    {callout(
        ...
        "Строчка над функцией — <strong>декоратор</strong>. "
...
def build_29() -> None:
    ...
    приложения. <code class="inline">flash(...)</code> сохраняет сообщение, которое покажется
    на следующей странице после редиректа — как именно это устроено, показывает следующий
    раздел, 22.30. <code class="inline">clean_title(...)</code> — простая функция проверки из
...
def build_32() -> None:
    ...
    {callout(
        ...
        "Метод <code class=\"inline\">sqlite3.execute()</code> в Python не разрешает несколько "
        "SQL-инструкций за один вызов: составная атака вроде "
        "<code class=\"inline\">x'; DROP TABLE tasks; --</code> завершится ошибкой "
        "<code class=\"inline\">ProgrammingError</code>. Это ограничение конкретного "
        "Python-драйвера, а не универсальная защита SQL. Оно не спасает от одноинструкционных "
        "инъекций: ввод <code class=\"inline\">' OR '1'='1</code> меняет условие "
        "<code class=\"inline\">WHERE</code> и возвращает все строки. Никогда не вставляйте "
