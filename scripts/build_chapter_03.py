...
def sidebar(active_href: str) -> list[SidebarGroup]:
    ...
    return [
        ...
        SidebarGroup("Практика", [
            ...
            NavItem("🐍 03-05: Практика", "../../practice/03-05/index.html"),
            NavItem("🐍 03-06: Практика", "../../practice/03-06/index.html"),
            NavItem("🐍 03-07: Практика", "../../practice/03-07/index.html"),
            NavItem("🐍 03-08: Практика", "../../practice/03-08/index.html"),
            NavItem("🐍 03-09: Практика", "../../practice/03-09/index.html"),
            NavItem("🐍 03-10: Практика", "../../practice/03-10/index.html"),
        ]),
    ]
...
def build_opener() -> None:
    out = render_chapter_opener(
        ...
        meta_items=["⏱ ~2.5 часа", "💻 VS Code, PyCharm или браузер", "📓 10 ноутбуков практики"],
        sections=[
...
{callout(
        "warning",
        "⚠️ Про зелёный треугольник рядом с if __name__",
...
{callout(
        "warning",
        "⚠️ Почему все путают «shell» и «Shell»",
...
{classic_vs_modern(
    ...
    <h2 id="lokalnaya-lab">🖥 Локальная лаборатория: попробуйте PySH (необязательно)</h2>
    ...
    <div class="exercise">
      <div class="exercise-stars">🖥 Необязательно · локально на вашем компьютере</div>
...
{callout(
        "warning",
        "⚠️ input() всегда возвращает текст",
...
{code_block("names.py", 'city = "Warsaw"\nprint(city)\n')}
    ...
    <h2 id="odno-znachenie-neskolko-imen">Одно значение — несколько имён</h2>
    <p>Поскольку имя — это просто указатель, у одного и того же значения может быть несколько
    имён одновременно:</p>
{code_block("names2.py", 'city = "Warsaw"\ntown = city\nprint(city)\nprint(town)\n')}
    <p>А присваивание новому имени НЕ меняет то, на что указывает старое:</p>
{code_block("names3.py", 'city = "Warsaw"\ntown = city\ncity = "Krakow"\nprint(city)  # Krakow\nprint(town)  # всё ещё Warsaw\n')}

    <h2 id="prostaya-model-pamyati">Простая модель: пространство имён процесса</h2>
    <p>Пока программа работает, Python хранит где-то список всех созданных имён и того, на что
    каждое из них сейчас указывает — это называется <strong>пространством имён</strong>
    (namespace). Схематично:</p>
{code_block(
        "Пространство имён (упрощённо)",
        "namespace\n├── city --> \"Warsaw\"\n└── town --> \"Warsaw\"",
        lang="text",
    )}
...
{callout(
        "info",
        "💡 Пока — без внутренностей",
        "Как именно Python хранит объекты в памяти (куча, счётчик ссылок, сборщик мусора) — тема "
        "для гораздо более поздних глав. Сейчас достаточно одной идеи: пока программа "
        "работает, Python поддерживает набор имён, которые указывают на объекты.",
    )}
...
{callout(
        "warning",
        "⚠️ Порядок запуска ячеек — это не порядок ячеек на экране",
