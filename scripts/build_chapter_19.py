...
def build_opener() -> None:
    out = render_chapter_opener(
        ...
        sections=[
            ChapterSectionLink("19.1", "Игра «Змейка»", "19-01-igra-import.html"),
            ChapterSectionLink("19.2", "Настраиваем экран Turtle", "19-02-ekran-peremennye.html"),
            ChapterSectionLink("19.3", "Рисуем голову", "19-03-golova-yabloko.html"),
            ChapterSectionLink("19.4", "Клавиши и движение головы", "19-04-klavishi-dvizhenie.html"),
            ChapterSectionLink("19.5", "Запускаем табло счёта", "19-05-tablo-scheta.html"),
            ChapterSectionLink("19.6", "Наша змейка ест!", "19-06-eda-telo.html"),
            ChapterSectionLink("19.7", "Проверка столкновений", "19-07-stolknoveniya.html"),
            ChapterSectionLink("19.8", "Полный код первого прототипа", "19-08-polnyj-kod-itogi.html"),
            ChapterSectionLink("19.9", "Мир игры как сетка", "19-09-mir-kak-setka.html"),
            ChapterSectionLink("19.10", "Координаты клетки и пиксели Turtle", "19-10-koordinaty-kletki.html"),
            ChapterSectionLink("19.11", "Направление как вектор", "19-11-napravlenie-kak-vektor.html"),
            ChapterSectionLink("19.12", "Один игровой тик", "19-12-odin-igrovoj-tik.html"),
            ChapterSectionLink("19.13", "Настоящий игровой цикл", "19-13-nastoyaschij-cikl.html"),
            ChapterSectionLink("19.14", "Время, скорость и задержка", "19-14-vremya-skorost.html"),
            ChapterSectionLink("19.15", "Состояние игры", "19-15-sostoyanie-igry.html"),
            ChapterSectionLink("19.16", "Голова, тело и модель Snake", "19-16-model-snake.html"),
            ChapterSectionLink("19.17", "Почему тело движется с хвоста", "19-17-pochemu-s-hvosta.html"),
            ChapterSectionLink("19.18", "Еда и свободная клетка", "19-18-eda-svobodnaya-kletka.html"),
            ChapterSectionLink("19.19", "Столкновение со стеной", "19-19-stolknovenie-so-stenoj.html"),
            ChapterSectionLink("19.20", "Столкновение с собой", "19-20-stolknovenie-s-soboj.html"),
            ChapterSectionLink("19.21", "Game Over как состояние", "19-21-game-over.html"),
            ChapterSectionLink("19.22", "Pause / Resume", "19-22-pauza.html"),
            ChapterSectionLink("19.23", "Restart / New Game", "19-23-restart.html"),
            ChapterSectionLink("19.24", "Скорость и рост сложности", "19-24-skorost-slozhnost.html"),
            ChapterSectionLink("19.25", "High Score", "19-25-high-score.html"),
            ChapterSectionLink("19.26", "Чистая игровая логика без Turtle", "19-26-chistaya-logika.html"),
            ChapterSectionLink("19.27", "GameState с dataclass", "19-27-gamestate-dataclass.html"),
            ChapterSectionLink("19.28", "SnakeApp: отделяем модель от визуализации", "19-28-snakeapp-arhitektura.html"),
            ChapterSectionLink("19.29", "Render: модель → Turtle", "19-29-render-model.html"),
            ChapterSectionLink("19.30", "Тестируем правила игры", "19-30-testiruem-pravila.html"),
            ChapterSectionLink("19.31", "Debug Labs", "19-31-debug-labs.html"),
            ChapterSectionLink("19.32", "Snake Pro — финальная игра", "19-32-snake-pro-itogi.html"),
            ChapterSectionLink("19.33", "Итоги главы", "19-33-itogi-glavy.html"),
        ],
    )
...
def build_02() -> None:
    ...
    {image_figure(f"{IMG}/snake-basic-empty-field.png", "Реальное окно: чёрный игровой экран 600×600 с надписью «Счёт: 0» и одним красным яблоком", "Реальное окно: экран настроен и переменные заведены. Яблоко и табло на снимке появятся в коде только в разделах 19.3 и 19.5 — здесь важен сам настроенный экран.", width=380)}
...
def build_03() -> None:
    ...
    {callout(
        ...
        "У <code class=\"inline\">randrange()</code> правая граница <em>исключается</em> — "
        "<code class=\"inline\">randrange(-280, 280, 20)</code> выдаёт значения только до 260 "
        "включительно. Змейка при этом спокойно доезжает до 280 (раздел 19.19), так что без "
        "<code class=\"inline\">+ RAZMER_SHAGA</code> крайний ряд клеток справа и сверху был бы "
        "для яблока недостижим — поле получилось бы незаметно несимметричным.",
    )}
...
def build_05() -> None:
    ...
    {callout(
        ...
        "<code class=\"inline\">tablo.goto(0, 260)</code> ставит надпись на координату "
        "<code class=\"inline\">y = 260</code>, а это <em>легальная клетка</em>: змейка может "
        "проехать прямо сквозь текст. Для первого прототипа это терпимо, но раздел 19.32 вынесет "
        "табло в отдельную полосу HUD над полем — туда, куда змейка по правилам попасть не может.",
    )}
...
def build_07() -> None:
    ...
    {callout(
        ...
        "Сама встроенная функция <code class=\"inline\">abs()</code> знакома по разделу 5.4 — "
        "здесь она впервые работает не как «модуль числа», а как способ сравнить расстояние до "
        "границы независимо от знака координаты.",
    )}
...
def build_08() -> None:
    ...
    {callout(
        ...
        "Цикл <code class=\"inline\">while</code> крутится со скоростью Python, а не со скоростью "
        "человека: от центра до стены всего 14 шагов, и без паузы они проходят примерно за две "
        "тысячных секунды — окно успело бы только моргнуть надписью «Игра окончена! Счёт: 0». "
        "<code class=\"inline\">time.sleep(0.12)</code> задаёт человеческий темп: примерно восемь "
        "шагов в секунду. Здесь <code class=\"inline\">sleep()</code> уместен именно потому, что "
        "цикл и так блокирующий — пока он работает, в программе всё равно ничего другого не "
        "происходит, а <code class=\"inline\">screen.update()</code> в конце каждого шага успевает "
        "разобрать накопившиеся нажатия клавиш. В архитектуре на "
        "<code class=\"inline\">screen.ontimer()</code> (раздел 19.13) тот же самый "
        "<code class=\"inline\">sleep()</code> уже станет ошибкой — Debug Lab 4 в разделе 19.31 "
        "разбирает, почему.",
    )}
    ...
    {callout(
        ...
        "Но <code class=\"inline\">while igrovoj_shag(): ...</code> — блокирующий цикл с "
        "фиксированной задержкой: скорость нельзя менять по ходу партии, а паузу и рестарт "
        "пришлось бы вплетать прямо в тело цикла. Всё состояние при этом живёт в глобальных "
        "переменных модуля. "
...
def build_09() -> None:
    ...
    одной из них. Рядом со словом «сетка» дальше в главе встретится и «решётка» — это одно и то
    же: узлы решётки и есть клетки сетки.</p>
...
def build_13() -> None:
    ...
    <p>Первый прототип запускает игру через <code class="inline">while igrovoj_shag(): ...</code>
    с <code class="inline">time.sleep()</code> в теле — простой и понятный блокирующий цикл.
    Здесь легко сделать неверный вывод, так что стоит проговорить его отдельно:
    <code class="inline">screen.update()</code> внутри цикла ДЕЙСТВИТЕЛЬНО обрабатывает
    накопившиеся события Tkinter, включая нажатия клавиш — значит, обработчик, привязанный через
    <code class="inline">onkeypress()</code>, технически может сработать прямо во время этого
    вызова. Проблема такого цикла не в том, что клавиатура «не успевает» — а в том, что он
    <em>владеет</em> управлением от начала и до конца партии и ни у кого не спрашивает
    разрешения. Скорость в нём — одна константа <code class="inline">ZADERZHKA_SEK</code>,
    ...
    заниматься ничем другим: она сидит внутри <code class="inline">sleep()</code>.</p>
    {code_block(
        "ontimer_cikl.py",
        "def game_tick(self):\n"
    ...
    Здесь есть тонкость: <code class="inline">ontimer()</code> только планирует будущий вызов —
    он не удаляется сам по себе, если статус игры поменяется. Раздел 19.22 показывает, почему
    поэтому пауза не может полагаться на одну лишь проверку <code class="inline">status</code>
    внутри тика — и что происходит, если положиться только на неё.</p>
    ...
    out = render_page(
        ...
        description="screen.ontimer() вместо блокирующего цикла while — тики планируются друг за другом, не блокируя цикл событий.",
        depth=2,
...
def build_14() -> None:
    ...
    {comparison_table(
        ...
        [
            ...
            ["140", "нормальная скорость — значение по умолчанию (BASE_DELAY_MS)"],
...
def build_15() -> None:
    ...
    да/нет. Финальная версия различает четыре состояния обычной партии, и переходы между ними —
    ...
    {callout(
        ...
        "В <code class=\"inline\">GameStatus</code> финальной программы пять членов, а не четыре: "
        "кроме четырёх состояний обычной партии есть <code class=\"inline\">WON</code> — змейка "
        "заняла буквально каждую клетку поля. Оно почти недостижимо в реальной игре, но это "
        "полноценное терминальное состояние, а не «почти GAME_OVER»: раздел 19.18 показывает, "
        "откуда оно берётся, а раздел 19.21 — чем терминальные состояния похожи друг на друга.",
    )}
    {callout(
        "tip",
...
def build_18() -> None:
    ...
    <p>В разделе 19.3 клетка для яблока выбиралась без проверки — при достаточно длинной змейке яблоко
    ...
    <h2>Инвариант: еда — только из свободных клеток</h2>
    <p>Контракт <code class="inline">choose_food()</code> можно записать формулой:
    <code class="inline">food ∈ FREE_CELLS</code>, где
    <code class="inline">FREE_CELLS = ALL_CELLS − SNAKE_CELLS</code>. Это должно оставаться
    верным ВСЕГДА, включая крайний случай, до которого редко доходят на практике, но который
    обязан быть предусмотрен: змейка выросла настолько, что заняла буквально каждую клетку
    поля. Тогда <code class="inline">FREE_CELLS</code> пусто, и честного ответа, где поставить
    еду, не существует.</p>
    {pipeline_diagram(
        [
            {"kind": "plain", "title": "ALL_CELLS", "rows": ["все клетки поля"]},
            {"kind": "plain", "title": "минус SNAKE_CELLS", "rows": ["клетки, занятые змейкой"], "note": "вычитание"},
            {"kind": "object", "title": "FREE_CELLS", "rows": ["food ∈ FREE_CELLS"], "note": "="},
        ],
        caption="Если FREE_CELLS пусто — свободной клетки для еды физически не существует.",
    )}
    {callout(
        "warning",
        "Нельзя тайком подставить клетку змейки вместо None",
        "Соблазнительное, но неверное решение — вернуть, например, "
        "<code class=\"inline\">snake[0]</code>, когда свободных клеток не осталось: тогда "
        "<code class=\"inline\">food == snake[0]</code>, и инвариант «еда никогда не внутри "
        "змейки» нарушается прямо в самом же коде, который эту еду выбирает. Честный ответ — "
        "<code class=\"inline\">None</code>: свободной клетки для еды в самом деле нет, и "
        "вызывающий код (раздел 19.32) обязан явно это обработать, а не притворяться, что еда "
        "по-прежнему существует.",
    )}
    <p>В <code class="inline">SnakeApp.game_tick()</code> это выглядит так: если змейка съела
    еду и <code class="inline">choose_food()</code> вернула <code class="inline">None</code>,
    игра переходит в терминальное состояние <code class="inline">GameStatus.WON</code> —
    поле заполнено целиком, дальше двигаться некуда, и это победа, а не ошибка.
    <code class="inline">state.food</code> в этом состоянии тоже становится
    <code class="inline">None</code>, и отрисовка (раздел 19.29) обязана явно проверить это
    перед тем, как рисовать еду на экране.</p>
    {image_figure(f"{IMG}/snake-won.png", "Реальное окно: игровое поле целиком заполнено телом змейки, сверху табло со счётом, по центру надпись ПОБЕДА и итоговый счёт", "Реальное окно после того, как змейка честно заняла каждую легальную клетку поля — GameStatus.WON, а не еда внутри тела.", width=460)}
...
def build_19() -> None:
    ...
    {callout(
        ...
        "доехать до самого края поля. Раздел 19.30 разбирает эти пограничные случаи как отдельные "
...
def build_21() -> None:
    ...
    <p><code class="inline">GAME_OVER</code> — не единственный терминальный статус: раздел 19.18
    показывает второй, куда более редкий случай — <code class="inline">GameStatus.WON</code>,
    когда змейка занимает буквально всё поле. Оба останавливают тики одним и тем же способом,
    отличается только причина и текст оверлея.</p>
...
def build_22() -> None:
    ...
    <h2>Почему одной проверки status недостаточно</h2>
    <p><code class="inline">screen.ontimer(callback, delay_ms)</code> только ПЛАНИРУЕТ будущий
    вызов — сам факт, что <code class="inline">status</code> сменился на
    <code class="inline">PAUSED</code>, никак не удаляет уже запланированный
    <code class="inline">callback</code> из очереди Tkinter: он всё ещё будет вызван, просто
    ...
    возобновление (<code class="inline">Resume</code>) произойдёт РАНЬШЕ, чем сработает вызов
    тика, запланированный ещё до паузы?</p>
    {callout(
        "debug",
        icon_label("debug", "Реальный баг: Pause → Resume до того, как сработал старый тик"),
        "Без инвалидации поколения именно в момент паузы получается: старый тик всё ещё "
        "запланирован → Resume планирует ВТОРОЙ, свой собственный тик → оба тика рано или "
        "поздно срабатывают, каждый планирует следующий сам за себя → в игре тикают ДВЕ "
        "независимые цепочки одновременно, и змейка начинает двигаться вдвое чаще, чем должна. "
        "Проверка <code class=\"inline\">status is RUNNING</code> внутри тика эту гонку не "
        "ловит: к моменту, когда старый тик наконец срабатывает, игра уже снова RUNNING — "
        "проверка проходит, и тик как ни в чём не бывало планирует продолжение.",
    )}
    <p>Поэтому <code class="inline">toggle_pause()</code> увеличивает
    <code class="inline">self._generation</code> на КАЖДОМ переключении — и при постановке на
    паузу, и при возобновлении, — а не только при перезапуске. Пауза инвалидирует
    запланированный тик сразу, в момент нажатия <code class="inline">Space</code>, а не
    ...
    существовала раньше, она уже мертва.</p>
    {pipeline_diagram(
        [
            {"kind": "object", "title": "TIMER A запланирован", "rows": ["generation = 4"]},
            {"kind": "plain", "title": "PAUSE", "rows": ["generation: 4 → 5"], "note": "Space"},
            {"kind": "plain", "title": "RESUME", "rows": ["generation: 5 → 6", "TIMER B запланирован (gen=6)"], "note": "Space, раньше, чем сработал TIMER A"},
            {"kind": "memory", "title": "TIMER A срабатывает", "rows": ["его generation = 4", "4 ≠ 6 → игнорируется, ничего не планирует"], "note": "поздно"},
            {"kind": "object", "title": "TIMER B срабатывает", "rows": ["его generation = 6", "6 = 6 → настоящий тик", "планирует следующий тик (gen=6)"], "note": "вовремя"},
        ],
        caption="Пауза и возобновление начинают новое поколение сразу, в момент нажатия Space — просроченный TIMER A узнаёт об этом при первой же проверке и не планирует ничего дальше.",
    )}
...
def build_23() -> None:
    ...
    <p>Restart не просто возвращает змейку в исходную позицию — раздел 19.22 предупреждал:
    если старая цепочка <code class="inline">ontimer()</code> всё ещё тикает, у игры на
    мгновение оказалось бы две независимые цепочки обновлений одновременно (глава 16 уже
    ...
    out = render_page(
        ...
        kicker="Глава 19 · Функции игры",
        h1="Restart / New Game",
        lede="Новая игра обязана оборвать старую цепочку тиков — иначе на мгновение окажутся "
        "две независимые цепочки тиков сразу.",
        body_html=body,
...
def build_24() -> None:
    ...
    {image_figure(f"{IMG}/snake-fast-speed.png", "Реальное окно: змейка из пяти сегментов, счёт 450, яблоко в верхней части поля", "Реальное окно: при высоком счёте calculate_delay() уже вернула минимальное значение — игра идёт на предельной скорости.", width=420)}
...
def build_26() -> None:
    ...
    {capability_map([
        ("next_head(head, direction)", ["кортеж → кортеж", "раздел 19.11"]),
...
def build_27() -> None:
    ...
    {class_diagram(
        ...
        ["snake: list[Position]", "direction: Direction", "next_direction: Direction",
         "food: Position | None", "score: int", "high_score: int", "status: GameStatus", "delay_ms: int"],
        [],
    ...
    {callout(
        ...
        "На девяноста девяти играх из ста <code class=\"inline\">food</code> — обычная клетка. "
        "Но раздел 19.18 показывает честный крайний случай: если змейка займёт буквально всё "
        "поле, свободной клетки для новой еды не останется. Тип <code class=\"inline\">Position "
        "| None</code> делает этот случай видимым в самой сигнатуре, а не спрятанным — код, "
        "который читает <code class=\"inline\">state.food</code>, обязан явно решить, что "
        "делать, если еды нет.",
    )}
    {callout(
        "warning",
    ...
    поле неспроста: раздел 19.31 (Debug Labs 5–6) объясняет, зачем клавиатура запрашивает
    направление, а не меняет его напрямую.</p>
...
def build_28() -> None:
    ...
    {comparison_table(
        ...
        [
            ...
            ["<code class=\"inline\">game_tick</code>", "один тик игры → изменения state"],
...
def build_29() -> None:
    ...
    <p>Змейка растёт, и напрашивается простое решение: создавать новый
    <code class="inline">turtle.Turtle()</code> на каждое съеденное яблоко — ровно так и делал
    первый прототип (раздел 19.6). Но объекты Turtle никуда не деваются: после
    <code class="inline">restart()</code> их окажется столько же, сколько было в самой длинной
    партии, и каждый из них экран продолжит обслуживать. Поэтому вводим <strong>пул
    сегментов</strong>: один раз созданные черепашки переиспользуются от кадра к кадру, новые
    добавляются только когда их действительно не хватает, а лишние прячутся, а не удаляются.</p>
    {code_block(
        "render.py",
        "def render(self):\n"
    ...
    {callout(
        ...
        "Раздел 19.18 обещал, что отрисовка честно обработает случай, когда свободной клетки для "
        "еды не осталось, — вот это место. Без проверки "
        "<code class=\"inline\">self.food_turtle.goto(*self.state.food)</code> при "
        "<code class=\"inline\">food = None</code> падает с "
        "<code class=\"inline\">TypeError: turtle.TNavigator.goto() argument after * must be an "
        "iterable, not NoneType</code> — ровно в момент победы, в самом редком и самом обидном "
        "состоянии игры.",
    )}
    {callout(
        "info",
...
def build_30() -> None:
    ...
    {callout(
        ...
        "ошибке ровно в этой точке (раздел 19.19 разбирал, почему граница включена).",
    )}
...
def build_31() -> None:
    ...
    {debug_lab(
        3,
        "Блокирующий цикл усложняет паузу и скорость",
        "busy_loop.py",
        "while igrovoj_shag():\n"
        "    screen.update()\n"
        ...
        ["# Игру нельзя поставить на паузу: событие обрабатывается,", "# но цикл ничего не проверяет и продолжает крутиться дальше.", "# Скорость задана одной константой и по ходу партии не меняется."],
        "Раздел 19.13 разбирал это подробно: <code class=\"inline\">screen.update()</code> внутри "
        "<code class=\"inline\">while</code> ДЕЙСТВИТЕЛЬНО обрабатывает события Tkinter, включая "
        "нажатия клавиш, — обработчик паузы технически может сработать. Но сам цикл не "
        "спрашивает, нужно ли ему остановиться: паузу, скорость и перезапуск пришлось бы "
        "вручную проверять на каждой итерации. <code class=\"inline\">screen.ontimer()</code> "
        "устроен иначе: каждый тик — это отдельный запланированный вызов, а не итерация одного "
        "бесконечного цикла, и решение «делать следующий тик или нет» принимается один раз, "
        "явно, а не проверяется заново внутри while.",
    ...
    {debug_lab(
        ...
        "    self.game_tick()\n",
        ["# Окно замирает НАВСЕГДА, а не на delay_ms: тик зовёт сам себя", "# без условия выхода, поэтому пауз становится бесконечно много,", "# а примерно через 1000 вызовов программа падает с RecursionError."],
        "Ошибок здесь сразу две. Первая: <code class=\"inline\">time.sleep()</code> блокирует ВЕСЬ "
        "процесс, включая цикл событий Tkinter, на котором держится Turtle. Вторая, менее "
        "заметная: <code class=\"inline\">game_tick()</code> вызывает саму себя напрямую, без "
        "всякого условия выхода — это обычная бесконечная рекурсия, и стек кончится раньше, чем "
        "игрок дождётся хоть одного отклика. <code class=\"inline\">screen.ontimer()</code> "
        "решает обе: он ничего не блокирует и не наращивает стек — раздел 19.13 объясняет "
        "разницу подробно.",
        "bez_sleep.py",
        "def game_tick(self):\n"
        "    # ...\n"
    ...
    {debug_lab(
        ...
        "Ловушка тонкая: проверка обязана идти против "
...
def build_33() -> None:
    ...
    {summary_box("Что мы построили и чему научились", [
        ...
        "того чтобы блокировать программу в цикле while с time.sleep().",
    ...
    out = render_page(
        ...
        kicker="Глава 19 · Финал",
        h1="Итоги главы",
        lede="От блокирующего цикла с глобальными переменными до модели состояния с игровым тиком, "
