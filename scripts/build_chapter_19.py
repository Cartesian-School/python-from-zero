...
def build_13() -> None:
    ...
    — простой и понятный busy-цикл. Здесь легко сделать неверный вывод, так что стоит
    проговорить его отдельно: <code class="inline">screen.update()</code> внутри цикла
    ДЕЙСТВИТЕЛЬНО обрабатывает накопившиеся события Tkinter, включая нажатия клавиш — значит,
    обработчик, привязанный через <code class="inline">onkeypress()</code>, технически может
    сработать прямо во время этого вызова. Проблема busy-цикла не в том, что клавиатура «не
    успевает» — а в том, что сам цикл ничего не планирует и ни у кого не спрашивает разрешения:
    он крутится так быстро, как только способен выполниться Python-код между двумя
    <code class="inline">update()</code>, без встроенного контроля скорости, а паузу, изменение
    темпа и перезапуск пришлось бы вручную вплетать в тело цикла отдельными флагами и
    проверками на каждой итерации.</p>
    {code_block(
        "ontimer_cikl.py",
        "def game_tick(self):\n"
...
def build_18() -> None:
    ...
    <code class="inline">None</code>, и отрисовка (раздел 19.32) обязана явно проверить это
...
def build_22() -> None:
    ...
    возобновление (<code class="inline">Resume</code>) произойдёт РАНЬШЕ, чем успеет сработать
    старый, ещё не выполнившийся тик?</p>
    {callout(
        "debug",
...
def build_31() -> None:
    ...
    {debug_lab(
        3,
        "Busy-цикл усложняет паузу и скорость",
        "busy_loop.py",
        "while igrovoj_shag():\n"
        "    screen.update()\n"
        "# цикл сам решает, когда именно проверять события — паузу, скорость\n"
        "# и перезапуск приходится вручную вплетать в его тело\n",
        ["# Игра не умеет ставиться на паузу: событие обрабатывается,", "# но цикл ничего не проверяет и продолжает крутиться дальше.", "# Заметна и постоянная нагрузка на процессор — цикл не отдыхает между тиками."],
