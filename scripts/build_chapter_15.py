...
def build_01() -> None:
    ...
    <p>Файл — это именованная последовательность данных, которую хранит файловая система и к
    которой программа обращается по <strong>пути</strong>. «Текстовый файл» — не какие-то особые
    буквы на диске: в конечном счёте любое хранилище содержит <strong>байты</strong>, а то, как
    именно байты превращаются в читаемые символы, определяет <strong>кодировка</strong> (об этом
    подробно — в разделе 15.16). Пока достаточно знать: файл хранит байты, а Python по указанному
    пути открывает файл и превращает его содержимое в привычные строки или сохраняет строки
    обратно в байты.</p>
...
def build_15() -> None:
    ...
    {branch_diagram(
        ...
        [
            ...
            ("\"w\"", "запись; создаёт или стирает файл сразу при открытии"),
            ("\"a\"", "дозапись в конец; создаёт, если файла нет"),
            ("\"x\"", "создать новый; ошибка, если файл уже есть"),
        ],
        caption="Четыре базовых режима — дальше добавляются модификаторы t/b/+.",
    )}
...
def build_16() -> None:
    ...
    page(
        ...
        description="str и bytes — разные типы; encode/decode превращают одно в другое; почему явный encoding=\"utf-8\" обязателен, и что происходит при несовпадении кодировок.",
...
def build_30() -> None:
    ...
    <ol>
      ...
      <li>Выполните ячейку, создающую папку <code class="inline">data/</code> и записывающую в
        неё UTF-8-файл.</li>
      <li>Останановите выполнение (или закройте редактор).</li>
...
def build_31() -> None:
    ...
    {capability_map([
        ("Персистентность", ["память процесса — временна", "файл на диске — переживает завершение", "загрузка ≠ тот же объект"]),
    ...
    {tree_diagram(
        ("Работа с файлами", [
            ("Персистентность", [("память временна", []), ("файл переживает программу", [])]),
    ...
    {summary_box("Что мы узнали в этой главе", [
        "Объекты Python живут в памяти процесса и не переживают его завершение — файл на диске переживает.",
