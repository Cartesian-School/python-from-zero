...
def build_09() -> None:
    ...
    {callout(
        "info",
        "Почему != , а не ≠",
        "В математике «не равно» пишут одним значком — <strong>≠</strong>. На клавиатуре "
        "такого значка нет, и Python (как и C, Java, JavaScript и почти все языки "
        "программирования) не понимает его как код — писать нужно ровно два обычных символа "
        "подряд: восклицательный знак и знак равенства, <code class=\"inline\">!=</code>. Это "
        "два разных, но эквивалентных обозначения одной и той же идеи: ≠ — запись для "
        "человека на бумаге, <code class=\"inline\">!=</code> — запись для Python. Если в коде "
        "на этом сайте <code class=\"inline\">!=</code> выглядит как единый перечёркнутый "
        "значок — это лишь особенность шрифта редактора (лигатура), которая красиво "
        "<em>отображает</em> два символа рядом; печатать при этом всё равно нужно "
        "<code class=\"inline\">!</code> и <code class=\"inline\">=</code> по отдельности — "
        "отдельного «значка ≠» на клавиатуре искать не нужно, его там нет.",
    )}
