...
def build_opener() -> None:
    out = render_chapter_opener(
        ...
        meta_items=["⏱ ~3 часа", "💻 Windows, macOS или Linux", "🔧 local-required — практика на вашем компьютере"],
        sections=[
            ChapterSectionLink("2.1", "Что именно мы устанавливаем?", "02-01-govorim-na-yazyke-kompyutera.html", "13"),
            ChapterSectionLink("2.2", "Терминал, shell и PATH", "02-04-terminal-shell-i-path.html", ""),
            ChapterSectionLink("2.3", "Установка Python на Windows", "02-02-windows.html", "14"),
            ChapterSectionLink("2.4", "Установка Python на macOS", "02-03-mac.html", "18"),
            ...
            ChapterSectionLink("2.17", "Рекомендации Cartesian School и итоги", "02-17-rekomendacii-i-itogi.html", "25"),
        ],
    )
...
{callout(
        "security",
        "⚠️ Скачивайте только с python.org",
...
{callout(
        "warning",
        "⚠️ Не трогайте системный Python",
...
{callout(
        "warning",
        "⚠️ PEP 668: «externally-managed-environment»",
...
{converge_diagram(
        ...
        caption="Выбранный вами интерпретатор обычно сохраняется в настройках именно этого проекта",
    )}

    <p>Когда вы выбираете интерпретатор через «Select Interpreter», VS Code обычно записывает
    выбор в <code class="inline">.vscode/settings.json</code> проекта — то есть выбор
    привязывается к проекту, а не «навсегда» ко всему компьютеру. Это нормально: у разных
    проектов на компьютере вполне может быть разный Python.</p>

{code_block(".vscode/settings.json", '{\n  "python.defaultInterpreterPath": "/home/anna/projects/my-project/.venv/bin/python"\n}', lang="json")}
...
{callout(
        ...
        "терминале — почти всегда причина в том, что выбран не тот интерпретатор. Проверьте "
        "«Select Interpreter» и путь в <code class=\"inline\">.vscode/settings.json</code>.",
    )}"""
...
def build_09_pycharm() -> None:
    ...
    Python, в отличие от VS Code, который универсален и понимает Python через расширения. У
    PyCharm есть бесплатная <strong>Community-редакция</strong> и платная
    <strong>Professional-редакция</strong> с дополнительными инструментами для веб-разработки и
    баз данных — для этого курса вполне достаточно бесплатной Community.</p>
...
{callout(
    ...
    (например, Jupyter-блокнотов — этот плагин тоже входит в Community-редакцию).</p>
...
{callout(
        ...
        "Python из коробки, — попробуйте PyCharm Community. Материалы курса не зависят от выбора "
        "редактора.",
    )}"""
...
{callout(
        "warning",
        "⚠️ Это не гипотетический сценарий",
...
{callout(
        "warning",
        "⚠️ Активация — на каждую новую вкладку/окно терминала",
...
{callout(
        "warning",
        "⚠️ Не смешивайте venv и conda для одного окружения",
...
def build_16_diagnostics() -> None:
    ...
    lab_html = "".join(
        ...
        <div class="callout callout-debug">
          <div>
            <div class="callout-title">🐞 Лаборатория {i}. {title}</div>
