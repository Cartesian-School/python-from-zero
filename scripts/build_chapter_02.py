...
def build_03_mac() -> None:
    body = f"""
    <p>На современном macOS уже есть команда <code class="inline">/usr/bin/python3</code> — но
    это не «ваш» Python, а <strong>Apple-контролируемый инструмент</strong>: он поставляется
...
{callout(
        "warning",
        "[[icon:warning]] Не трогайте Apple-Python",
        "Никогда не устанавливайте пакеты «поверх» <code class=\"inline\">/usr/bin/python3</code> "
        "и не удаляйте его — это официальная рекомендация docs.python.org: этот Python "
        "Apple-контролируемый и используется системным и сторонним ПО. Вместо этого поставьте "
        "отдельную, «вашу собственную» версию Python с python.org — так делают профессиональные "
        "разработчики, и оба Python спокойно сосуществуют на одном компьютере.",
    )}
...
{callout(
        ...
        "На современном macOS (и на Linux) команда <code class=\"inline\">python</code> без "
        "суффикса обычно вообще не существует. Так было не всегда: на очень старых версиях "
        "macOS (до Catalina, 2019 год) команда <code class=\"inline\">python</code> исторически "
        "указывала на системный Python 2 — но это осталось в прошлом, а не текущее поведение. "
        "Официальный установщик python.org ставит команду именно как "
        "<code class=\"inline\">python3</code>. Мы разберём, как сделать команду "
        "<code class=\"inline\">python</code> удобной внутри виртуального окружения, в разделе "
        "2.10.",
    )}
    ...
    вероятно, Apple-контролируемый <code class="inline">/usr/bin/python3</code>, а не тот
    интерпретатор, что вы только что установили; проверьте порядок PATH (раздел 2.4) или явно
    укажите путь через <code class="inline">sys.executable</code> (раздел 2.6), чтобы убедиться,
    какой именно <code class="inline">python3</code> реально запускается.</p>"""
