...
def build_08() -> None:
    ...
    {object_diagram(
        ...
        caption="Не каждое поле имеет смысл для каждого события — see ниже.",
    )}
    {callout(
        "warning",
...
def build_10() -> None:
    ...
    {callout(
        ...
        "семантическая активация: она срабатывает и по клику мышью, и по нажатию Enter/пробела, "
        "когда кнопка в фокусе (клавиатурная доступность — глава 16, раздел 16.21). Голая "
        "привязка <code class=\"inline\">&lt;Button-1&gt;</code> реагирует ТОЛЬКО на клик мышью "
        "— вы молча теряете клавиатурную активацию. Замена command на bind «просто потому что "
        "можно» — типичная архитектурная ошибка, не стилистическая мелочь.",
    )}
...
def build_11() -> None:
    body = f"""
    <h2>Клавиатурные события нужен адресат</h2>
    <p>Окно, в котором есть привязка к клавише, не значит, что нажатие ЛЮБОЙ клавиши где угодно
    попадёт в этот обработчик. Клавиатурные события идут в виджет, у которого сейчас
    <strong>фокус ввода</strong>.</p>
    {code_block(
        "focus_demo.py",
    ...
    {callout(
        "warning",
        "«Привязка не работает» часто означает «нет фокуса»",
        "Классическая ловушка: <code class=\"inline\">root.bind(\"&lt;Key&gt;\", ...)</code> "
        "написан правильно, но фокус находится на другом виджете (например, на поле ввода "
        "внутри диалога), и нажатия клавиш туда и уходят. Смотрите Debug Lab 4 (раздел 17.29).",
    )}
...
def build_16() -> None:
    ...
    {callout(
        ...
        "который их использует. Изменить размер поля правило-в-правило начинается с изменения "
        "этой константы, а не переписывания алгоритма.",
    )}
...
def build_17() -> None:
    full_win_diag = board_diagram(["O", "O", "X", "X", "X", "", "", "", "X"], highlighted=(2, 4, 6), caption="9-й ход одновременно заполняет поле И выигрывает диагональ 2-4-6.")
    body = f"""
...
def build_19() -> None:
    ...
    {callout(
        "warning",
        "НЕ board: list[str] = [\"\"] * 9",
        "Глава 14 уже объясняла эту ловушку: изменяемое значение по умолчанию "
        "(<code class=\"inline\">list</code>) в обычном присваивании расшарилось бы между "
        "ВСЕМИ экземплярами <code class=\"inline\">GameState</code> — испортив вторую игру "
        "остатками первой. <code class=\"inline\">field(default_factory=lambda: [\"\"] * 9)</code> "
        "создаёт новый список для каждого экземпляра.",
    )}
...
def build_20() -> None:
    ...
    {comparison_table(
        ...
        [
            ...
            ["index → row/column", "render() — единственное место, красящее виджеты"],
...
def build_21() -> None:
    ...
    {callout(
        "info",
        "sticky='nsew' + weight — из главы 16",
        "Те же приёмы адаптивного <code class=\"inline\">grid()</code>, что и в разделе 16.15: "
        "<code class=\"inline\">sticky=\"nsew\"</code> заставляет клетку заполнить всю ячейку "
        "сетки, а <code class=\"inline\">rowconfigure/columnconfigure(weight=1)</code> "
        "распределяет свободное пространство при изменении размера окна.",
    )}
    {callout(
        "warning",
...
def build_23() -> None:
    ...
    {image_figure(f"{IMG}/hover-preview-x.png", "Наведение на пустую клетку показывает бледный превью X", f"Реальное окно: курсор навёлся на пустую клетку — превью показывает X бледным цветом, ход ещё не сделан. Результат выполнения кода ниже.", width=320)}
    {code_block(
        "hover_preview.py",
...
def build_24() -> None:
    ...
    {callout(
        ...
        "введённый символ. Для цифровых клавиш он совпадает и с верхним рядом, и с "
        "дополнительной цифровой клавиатурой (NumPad) на большинстве систем — поэтому здесь он "
        "удобнее, чем сравнение по <code class=\"inline\">event.keysym</code>.",
    )}
...
def build_25() -> None:
    ...
    {image_figure(f"{IMG}/winning-highlight.png", "Диагональ 0-4-8 подсвечена зелёным, статус говорит Победил игрок X", f"Реальное окно: победная диагональ подсвечена, буквы остаются видимыми. Результат выполнения кода выше.", width=320)}
...
def build_26() -> None:
    body = f"""
    <h2>Счёт живёт в GameState, не в отдельной переменной where-попало</h2>
    {image_figure(f"{IMG}/scoreboard.png", "Строка счёта: X: 2, O: 1, Ничьи: 1", f"Реальное окно: счёт матча под игровым полем. Результат выполнения кода ниже.", width=320)}
    {code_block(
        "schet.py",
        "if winner == \"X\":\n"
        "    state.score_x += 1\n"
...
def build_27() -> None:
    ...
    {image_figure(f"{IMG}/new-round.png", "Поле пустое, статус Ход игрока X, но счёт X:1 O:0 сохранён", f"Реальное окно: раунд сброшен после победы X, но счёт матча остался. Результат выполнения кода выше.", width=320)}
...
def build_29() -> None:
    ...
    {debug_lab(
        4,
        "Клавиатурное событие не приходит — фокус не там",
        "focus_problem.py",
        "root.bind(\"<Key>\", on_key)\n"
        "entry = tk.Entry(root)\n"
        "entry.pack()\n"
        "entry.focus_set()   # фокус ушёл в поле ввода\n",
        ["# Нажатия цифр ничего не делают в игре —", "# они уходят в Entry, у которого сейчас фокус."],
        "Клавиатурные события идут строго туда, где сейчас фокус ввода (раздел 17.11). Если в "
        "приложении есть виджет, способный забрать фокус (поле ввода, кнопка после клика), "
        "привязка на <code class=\"inline\">root</code> не получит событие, пока фокус не "
        "вернётся на подходящий виджет.",
        "focus_fixed.py",
        "root.bind(\"<Key>\", on_key)\n"
        "root.focus_set()   # явно вернуть фокус на главное окно\n",
    )}
    ...
    {debug_lab(
        ...
        "изменения модели никогда не попадают на экран. Раздел 17.49 (глава-16-подобный "
        "принцип): render() — ЕДИНСТВЕННОЕ место, которое красит виджеты, и его нужно "
        "вызывать явно после каждого изменения модели.",
        "reset_s_render.py",
        "def new_round(self):\n"
        '    self.state.board = [""] * 9\n'
        '    self.state.current_player = "X"\n'
...
def build_30() -> None:
    ...
    {callout(
        "tip",
        "Никакого time.sleep(), никакой резкой стробоскопии",
        "Пауза между тиками — 150 мс, всего 6 тиков (меньше секунды). Этого достаточно, чтобы "
        "победа была замечена, но не настолько быстро, чтобы вызвать дискомфорт от мигания.",
    )}
...
def build_31() -> None:
    ...
    checklist_arch = "".join(f"<li>{x}</li>" for x in [
        ...
        "render() — единственный источник изменений на экране",
    ])
    body = f"""
    <h2>Итоговая программа</h2>
    {image_figure(f"{IMG}/tic-tac-toe-pro.png", "Финальное окно Tic-Tac-Toe Pro с игрой в процессе и счётом X:1 O:2 Ничьи:1", f"Реальное окно финальной версии — модель, правила, счёт, подсветка и клавиатура вместе. Результат выполнения кода ниже.", width=340)}
    ...
    {summary_box("Что мы построили и чему научились", [
        ...
        "render() — единственное место, которое красит виджеты; модель меняют callback-и, "
        "виджеты только отображают её.",
