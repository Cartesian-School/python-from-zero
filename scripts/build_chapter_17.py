...
def build_opener() -> None:
    out = render_chapter_opener(
        ...
        sections=[
            ChapterSectionLink("17.1", "Привязка событий — делаем приложения динамическими!", "17-01-privyazka-sobytij.html"),
            ChapterSectionLink("17.2", "Игра «Крестики-нолики» — объяснение", "17-02-obyasnenie-nastrojka.html"),
            ChapterSectionLink("", "Настраиваем Tkinter", "17-02-obyasnenie-nastrojka.html#nastrojka"),
            ChapterSectionLink("17.3", "Создаём глобальные переменные", "17-03-peremennye-knopki.html"),
            ChapterSectionLink("", "Создаём кнопки", "17-03-peremennye-knopki.html#knopki"),
            ChapterSectionLink("17.4", "При нажатии на кнопку рисуем на ней", "17-04-risuem-na-knopke.html"),
            ChapterSectionLink("17.5", "Проверяем после каждого хода, победил ли игрок", "17-05-proverka-pobedy.html"),
            ChapterSectionLink("17.6", "Новая игра и первая полная версия", "17-06-novaya-igra-itogi.html"),
            ChapterSectionLink("17.7", "Event, callback, command и binding", "17-07-event-callback-command-binding.html"),
            ChapterSectionLink("17.8", "Объект Event", "17-08-obyekt-event.html"),
            ChapterSectionLink("17.9", "Синтаксис событий Tk", "17-09-sintaksis-sobytij.html"),
            ChapterSectionLink("17.10", "command vs bind — что выбирать", "17-10-command-vs-bind.html"),
            ChapterSectionLink("17.11", "Focus и клавиатура", "17-11-focus-i-klaviatura.html"),
            ChapterSectionLink("17.12", "Mouse Enter/Leave и hover", "17-12-enter-leave-hover.html"),
            ChapterSectionLink("17.13", "Модель игрового состояния", "17-13-model-sostoyaniya.html"),
            ChapterSectionLink("17.14", "Индексы, строки и столбцы", "17-14-indeksy-stroki-stolbcy.html"),
            ChapterSectionLink("17.15", "Правильный алгоритм хода", "17-15-algoritm-hoda.html"),
            ChapterSectionLink("17.16", "Восемь выигрышных линий", "17-16-vosem-linij-pobedy.html"),
            ChapterSectionLink("17.17", "Победа, ничья и terminal state", "17-17-pobeda-nichya-terminal.html"),
            ChapterSectionLink("17.18", "От widgets-as-state к board model", "17-18-model-a-ne-widgets.html"),
            ChapterSectionLink("17.19", "GameState с dataclass", "17-19-gamestate-dataclass.html"),
            ChapterSectionLink("17.20", "Архитектура TicTacToeApp", "17-20-arhitektura-app.html"),
            ChapterSectionLink("17.21", "Адаптивное игровое поле", "17-21-adaptivnoe-pole.html"),
            ChapterSectionLink("17.22", "Визуальный стиль X/O", "17-22-vizualnyj-stil.html"),
            ChapterSectionLink("17.23", "Hover preview через bind()", "17-23-hover-preview.html"),
            ChapterSectionLink("17.24", "Управление клавиатурой", "17-24-upravlenie-klaviaturoj.html"),
            ChapterSectionLink("17.25", "Подсветка выигрышной линии", "17-25-podsvetka-linii.html"),
            ChapterSectionLink("17.26", "Счёт матчей", "17-26-schyot-matchej.html"),
            ChapterSectionLink("17.27", "New Round vs New Match", "17-27-new-round-vs-new-match.html"),
            ChapterSectionLink("17.28", "Тестируем игру без Tkinter", "17-28-testiruem-bez-tkinter.html"),
            ChapterSectionLink("17.29", "Debug Labs", "17-29-debug-labs.html"),
            ChapterSectionLink("17.30", "Visual effects и after()", "17-30-visual-effects-after.html"),
            ChapterSectionLink("17.31", "Tic-Tac-Toe Pro — итоги главы", "17-31-tic-tac-toe-pro-itogi.html"),
        ],
    )
...
def build_01() -> None:
    ...
    <p>Когда callback зарегистрирован через <code class="inline">.bind()</code>, Tkinter
    передаёт ему объект <code class="inline">Event</code> с подробностями о произошедшем событии:
    ...
    <code class="inline">command=</code> (как в главе 16), обычно вызывается без объекта Event —
    раздел 17.8 разберёт разницу подробнее.</p>

    {callout(
        "info",
        "Пригодится в главе 19",
        "Та же идея — связать клавишу с функцией — вернётся в главе 19: змейка будет реагировать "
        "на нажатия клавиш со стрелками. У Turtle для этого есть свой, более простой "
        "интерфейс — <code class=\"inline\">screen.listen()</code> и "
        "<code class=\"inline\">screen.onkeypress()</code>, — но принцип тот же самый.",
    )}
    ...
    out = render_page(
        ...
        "открывает доступ к событиям любого рода.",
        body_html=body,
...
def build_07() -> None:
    ...
    out = render_page(
        ...
        lede="«Кнопки Tkinter — это и есть игра» — неверная модель. Начнём с вопроса: кто вообще "
...
def build_08() -> None:
    ...
    out = render_page(
        ...
        kicker="Глава 17 · События глубже",
        h1="Объект Event",
        lede="Callback из bind() получает Event; callback из command= обычно вызывается без него. Разберём, что внутри "
        "этого объекта.",
        body_html=body,
...
def build_10() -> None:
    ...
    {callout(
        ...
        "семантическая активация: она описывает основное действие кнопки, а встроенное "
        "поведение класса Button решает, какие способы активации вызывают эту команду. "
        "Конкретные клавиши могут отличаться у <code class=\"inline\">tk.Button</code> и "
        "<code class=\"inline\">ttk.Button</code>, а также между платформами и темами. "
        "Голая привязка <code class=\"inline\">&lt;Button-1&gt;</code> описывает только конкретное "
        "событие мыши и обходит семантическое действие виджета. Замена command на bind "
    ...
    {capability_map([
        ("Ход по клетке", ["command= у Button", "основное действие виджета"]),
...
def build_12() -> None:
    ...
    {callout(
        ...
        "привязку целиком. Для главы 17 не нужны — но Debug Lab 17 (раздел 17.29) показывает, "
...
def build_13() -> None:
    ...
    {object_diagram(
        ...
        [("board", "['', '', '', '', '', '', '', '', '']"), ("current_player", "'X'"), ("game_over", "False"),
         ("winner", "None"), ("winning_line", "None"), ("score_x", "0"), ("score_o", "0"), ("draws", "0")],
        width=720,
    ...
    <h2>Одно состояние — разные видимые моменты партии</h2>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:14px;align-items:start">
      {image_figure(f"{IMG}/x-turn.png", "Пустое поле и статус Ход игрока: X", "Реальное состояние канонического приложения: начало раунда, ход X.", width=280)}
      {image_figure(f"{IMG}/o-turn.png", "После первого хода X статус сменился на Ход игрока: O", "Реальное состояние того же приложения после валидного хода X: модель переключила current_player на O.", width=280)}
      {image_figure(f"{IMG}/mid-game.png", "Пять отметок X и O на поле, партия продолжается", "Реальное промежуточное состояние канонического приложения: board хранит отметки, status_var показывает текущего игрока.", width=280)}
    </div>
    ...
    out = render_page(
        ...
        lede="Tkinter показывает игру и передаёт ввод пользователя. Игровое СОСТОЯНИЕ — "
        "отдельная вещь.",
        body_html=body,
...
def build_15() -> None:
    ...
    {callout(
        ...
        "Это частая ошибка (Debug Lab 7, раздел 17.29): если клик проигнорирован — очередь "
    ...
    <h2>Видимый результат валидного хода</h2>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:14px;align-items:start">
      {image_figure(f"{IMG}/empty-board.png", "Пустое поле перед первым ходом", "Реальное окно канонического приложения до хода: модель board содержит девять пустых строк.", width=300)}
      {image_figure(f"{IMG}/x-first-move.png", "X занял центральную клетку, очередь перешла к O", "То же реальное приложение после attempt_move(4): отметка записана в модель, затем render() обновил поле и статус.", width=300)}
    </div>
    ...
    {callout(
        ...
        "проще начинать. Короткая версия (условное выражение, глава 9; его часто называют "
        "тернарным оператором) — не «умнее», просто компактнее, когда вы уже уверенно её "
        "читаете.",
    )}
...
def build_16() -> None:
    ...
    {callout(
        ...
        "<code class=\"inline\">WINNING_LINES</code> написана ЗАГЛАВНЫМИ буквами: так в Python "
        "принято называть константы модуля (соглашение PEP 8 — договорённость между "
        "программистами, а не запрет интерпретатора на изменение). Это данные о правилах игры, отдельные от кода, "
...
def build_17() -> None:
    ...
    <h2>Три терминальных результата в реальном окне</h2>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:14px;align-items:start">
      {image_figure(f"{IMG}/x-win.png", "X выиграл верхнюю строку", "Реальное состояние полной версии: победа X, выигрышная линия подсвечена, поле заблокировано.", width=280)}
      {image_figure(f"{IMG}/o-win.png", "O выиграл средний столбец", "Реальное состояние полной версии: та же логика find_winner() работает для O.", width=280)}
      {image_figure(f"{IMG}/draw.png", "Полное поле без выигрышной линии, статус Ничья", "Реальное состояние полной версии: поле заполнено, победителя нет, дополнительные ходы заблокированы.", width=280)}
    </div>
...
def build_21() -> None:
    body = f"""
    <h2>Поле не должно превращаться в хрупкую конструкцию из виджетов с фиксированным размером</h2>
    {code_block(
        "adaptivnoe_pole.py",
...
def build_22() -> None:
    ...
    {callout(
        ...
        "статус явно пишет «Победил игрок X!». Это важное правило доступности: нельзя "
        "полагаться только на цвет как на единственный сигнал.",
    )}
...
def build_23() -> None:
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:14px;align-items:start">
      {image_figure(f"{IMG}/hover-preview-x.png", "Наведение на пустую клетку показывает бледный превью X", "Реальное окно полной версии: ход X, временный превью не изменяет board. Фрагмент ниже отвечает только за эффект наведения.", width=320)}
      {image_figure(f"{IMG}/hover-preview-o.png", "После хода X наведение показывает бледный превью O", "Реальное окно той же версии на ходе O: визуальный слой меняется вместе с current_player, но пустая клетка в модели остаётся пустой.", width=320)}
    </div>
    {code_block(
        "hover_preview.py",
    ...
    {callout(
        ...
        "Debug Lab 13 (раздел 17.29) разбирает это подробно. Правильный код всегда "
...
def build_24() -> None:
    ...
    {callout(
        ...
        "правил рано или поздно разошлись бы (Debug Lab 9, раздел 17.29 — похожая ошибка с "
        "опечаткой в линии).",
    )}
    <table class="compare-table">
...
def build_26() -> None:
    body = f"""
    <h2>Счёт живёт в GameState, а не в случайной отдельной переменной</h2>
    ...
    {callout(
        ...
        "Красивый символ-маркер «•» в headless-окружениях с урезанным набором шрифтов иногда "
        "превращается в набор мусорных символов — как выяснилось при "
...
def build_27() -> None:
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:14px;align-items:start">
      {image_figure(f"{IMG}/new-round.png", "После New Round поле пустое, а счёт X:1 сохранён", "Реальное окно полной версии после new_round(): состояние раунда сброшено, счёт матча сохранён. Код выше — фрагмент методов полного приложения.", width=320)}
      {image_figure(f"{IMG}/new-match.png", "После New Match поле пустое и весь счёт равен нулю", "Реальное окно полной версии после new_match(): сброшены и раунд, и счёт X/O/ничьих. Код выше — фрагмент методов полного приложения.", width=320)}
    </div>
...
def build_28() -> None:
    ...
    {callout(
        ...
        "Тест только для одной строки не поймает опечатку в описании диагонали — Debug Lab 9 "
...
def build_29() -> None:
    ...
    {debug_lab(
        15,
        "Голый <Button-1> подменяет семантическое действие Button",
        "goloj_button1.py",
        'btn.bind("<Button-1>", lambda e, i=index: attempt_move(i))\n'
        "# command= не задан вообще\n",
        ["# Обработчик связан только с конкретным событием мыши.", "# Основное действие Button больше не описано через command=."],
        "Раздел 17.10: <code class=\"inline\">command=</code> описывает основное действие "
        "Button, а встроенное поведение класса виджета решает, какие способы активации его "
        "вызывают. Конкретные клавиши зависят от вида Button, темы и платформы. Голая "
        "привязка <code class=\"inline\">&lt;Button-1&gt;</code> знает только о клике мышью и "
        "обходит эту семантику — поэтому не используйте её вместо command по умолчанию.",
        "command_fixed.py",
    ...
    {debug_lab(
        ...
        "        time.sleep(0.15)\n",
        ["# Окно перестаёт отвечать почти на две секунды анимации —", "# та же ошибка, что и в главе 16 (раздел 16.32)."],
...
def build_30() -> None:
    ...
    <p>После победы клетки выигрышной линии один раз получают мягкий акцент и спокойно
    возвращаются к постоянной подсветке. На реальном окне это выглядит как краткий переход
    между accent-состоянием и базовой подсветкой победы; сайт не может показать
    видео-анимацию, поэтому ниже — реальные кадры этого перехода, а локальное приложение
    переключает их через <code class="inline">after()</code>. Различимых кадров всего два:
    третий снимок совпадает со вторым не случайно — эффект заканчивается именно на постоянной
    подсветке победы и дальше ничего не меняет.</p>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;align-items:start">
      {image_figure(f"{IMG}/win-pulse-step-0.png", "Начало эффекта: выигрышная линия в мягком accent-цвете PULSE_BG", "Реальный первый кадр: мягкий акцент PULSE_BG.", width=260)}
      {image_figure(f"{IMG}/win-pulse-step-1.png", "Через 400 мс линия возвращается к постоянной подсветке WIN_BG", "Реальный второй кадр: постоянная подсветка WIN_BG.", width=260)}
      {image_figure(f"{IMG}/win-pulse-final.png", "После эффекта линия остаётся в постоянном цвете подсветки", "Установившееся состояние: картинка совпадает со вторым кадром, потому что эффект на нём и заканчивается.", width=260)}
    </div>
    ...
    {callout(
        ...
        "<code class=\"inline\">after_cancel()</code> перед сбросом — та же дисциплина трёх "
        "состояний, что и у таймера в разделе 16.28.",
    )}
    {code_block(
        "cancel_pulse.py",
    ...
    {callout(
        ...
        "Эффект не мигает серией: линия получает мягкий accent-цвет и через 400 мс остаётся в "
        "постоянной подсветке победы. Даже короткие эффекты должны быть ненавязчивыми и не "
        "отвлекать от статуса «Победил игрок X/O». Для продукта с формальными требованиями "
        "доступности параметры и возможность отключения эффектов проверяют отдельно.",
    )}
...
def build_31() -> None:
    ...
    {code_block(
        ...
        "def find_winner(board): ...\n"
        "def is_draw(board): ...\n"
        "def load_scores(): ...        # необязательное сохранение счёта (17.27)\n"
        "def save_scores(state): ...\n\n"
        "@dataclass\n"
        "class GameState:\n"
        "    ...\n\n"
        ...
        "    def new_match(self): ...\n"
        "    def pulse_winning_line(self, tick=0): ...\n"
        "    def cancel_pulse(self): ...      # отменяет запланированный after() (17.30)\n"
        "    def on_close(self): ...\n\n"
        "def main():\n"
        "    root = tk.Tk()\n"
    ...
    {summary_box("Что мы построили и чему научились", [
        ...
        "callback из command= обычно не получает Event; callback, зарегистрированный через bind(), получает его.",
        "command= описывает основное действие Button; bind() — явную реакцию на конкретное "
        "событие мыши или клавиатуры. Встроенные способы активации Button зависят от класса "
        "виджета и платформы, поэтому command и bind не следует считать взаимозаменяемыми.",
        ...
        "render() задаёт каноническое базовое отображение ИЗ модели. Наведение мыши и короткий "
        "акцент победы временно меняют представление поверх него, не меняя GameState; следующий "
        "render() снова полностью восстанавливает вид из модели.",
